...
int main(int argc, char *argv[]) {
    ...
    segment_8x8_centered_replicate_ADSP_opt(
        Rch, WIDTH, HEIGHT, blocks, &NBx, &NBy, &Wp, &Hp);
    ...
    encode_quant_blocks_to_file("out_R.dat", quant_blocks_sdram,
                                num_blocks, WIDTH, HEIGHT, NBx, NBy);
    STOP_CYCLE_COUNT(final_count, start_count);
   ...
   // printf("Blocks @ %p\n", (void*)blocks);
    ...
    printf("Segmented into NBx=%d, NBy=%d, total blocks=%d, padded %dx%d\n",
        ...
        START_CYCLE_COUNT(start_count);
        segment_8x8_centered_replicate_ADSP_opt(
            Gch, WIDTH, HEIGHT, blocks, &NBx, &NBy, &Wp, &Hp);
        num_blocks = NBx * NBy;
        dct8x8_blocks_to_sdram_float(blocks, num_blocks);
        quantize_dct_blocks_to_sdram_short(dct_blocks_sdram_f, num_blocks, NULL);
        encode_quant_blocks_to_file("out_G.dat", quant_blocks_sdram,
                                    num_blocks, WIDTH, HEIGHT, NBx, NBy);
        STOP_CYCLE_COUNT(final_count, start_count);
        ...
        printf("\n");
         ...
         START_CYCLE_COUNT(start_count);
         segment_8x8_centered_replicate_ADSP_opt(
             Bch, WIDTH, HEIGHT, blocks, &NBx, &NBy, &Wp, &Hp);
         num_blocks = NBx * NBy;
         dct8x8_blocks_to_sdram_float(blocks, num_blocks);
         quantize_dct_blocks_to_sdram_short(dct_blocks_sdram_f, num_blocks, NULL);
         encode_quant_blocks_to_file("out_B.dat", quant_blocks_sdram,
                                     num_blocks, WIDTH, HEIGHT, NBx, NBy);
         STOP_CYCLE_COUNT(final_count, start_count);
